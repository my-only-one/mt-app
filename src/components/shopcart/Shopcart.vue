<template>
  <div class="shopcart" :class="{'highligh':totalPrice>0}">
    <div class="shopcart-wrapper">
      <!--底部左侧-->
      <div class="content-left">
        <div class="logo-wrapper" :class="{'highligh':totalCount>0}" @click="toggleList">
          <span class="icon-shopping_cart logo" :class="{'highligh':totalCount>0}">
            <i class="num" v-show="totalCount">{{totalCount}}</i>
          </span>
        </div>
        <div class="desc-wrapper">
          <p class="total-price" v-show="totalPrice"
             :class="{'highligh':totalPrice>0}">￥{{totalPrice}}</p>
          <p class="tip" :class="{'highligh':totalPrice>0}">另需{{poiInfo.shipping_fee_tip}}</p>
        </div>
      </div>
      <!--底部右侧-->
      <div class="content-right" :class="{'highligh':totalPrice>0}">
        {{payStr}}
      </div>
      <!--购物车列表-->
