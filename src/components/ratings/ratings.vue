      ...
      <div class="content">
        ...
        <div class="rating-select" v-if="ratings.tab">
          <span class="item"
                ...
                @click="setTab(2)">{{ratings.tab[0].comment_score_title}}</span>
          <span class="item"
                :class="{'active': selectType === 1}"
                @click="setTab(1)">{{ratings.tab[1].comment_score_title}}</span>
          <span class="item"
        ...
        <div class="labels-view">
          <span class="item"
                ...
                :key="index"
                :class="{'heigligh': ratings.label_star > 0}">
            {{item.content}}{{item.label_count}}
          </span>
        </div>
        <!--评价列表-->
        <ul class="rating-list">
          <li v-for="(item, index) in selectComments" :key="index" class="comment-item">
            ...
            <div class="comment-main">
              ...
              <div class="star-wrapper">
                ...
                <app-star class='star' :score="item.order_comment_score"></app-star>
  ...
  created () {
    ...
    fetch('/api/ratings')
      ...
      .then(response => {
        ...
        this.$nextTick((v) => {
          if (!this.isScroll) {
            this.isScroll = new BScroll(this.$refs.ratingView, {
              click: true
            })
          } else {
  ...
  computed: {
    selectComments () {
      if (this.selectType === ALL) {
        ...
        let arr = []
        this.ratings.comments.forEach(v => {
          if (v.comment_pics.length) {
            arr.push(v)
          }
        })
        return arr
